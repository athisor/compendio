...
def sanitize_filename(name):
    # SWID should be lowercase, no special chars, hyphens instead of spaces
...
def parse_origins(text, lang):
    ...
    # Note: Regex logic is tailored to the specific formatting of the Summary MD files.
    ...
    for i in range(len(name_matches)):
        ...
        skip_terms = [
            "TRASFONDOS", "ORIGINS", "HABILIDADES", "EQUIPO", "SKILLS", "GEAR", "RAZAS", "SUMARIO DE TRASFONDOS",
            "CREDITOS", "INDICE", "INTRODUCCION", "CONTENIDOS", "GLOSARIO", "MALMART", "VEHICULOS", "CHICAGOLAND", "SECCION DEL DJ"
        ]
        if len(name.split()) > 5 or name.upper() in skip_terms: 
            continue
...
def main():
    ...
    root = r"d:\Rol\FoundryVTT\Data\modules\swade-setting-iz3\src\packs\iz3-origins"
